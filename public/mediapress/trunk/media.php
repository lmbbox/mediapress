...
if ( isset($_GET['find_detached'] ) ) {
	...
	$all_att = $wpdb->get_results("SELECT ID, post_parent FROM $wpdb->posts WHERE post_type = 'media'");
	...
	if ( ! empty($attach) ) {
		...
		$attached = $wpdb->query( $wpdb->prepare("UPDATE $wpdb->posts SET post_parent = %d WHERE post_type = 'media' AND ID IN ($attach)", $parent_id) );
	}
...
$title = __('MediaPress Library');
$parent_file = 'lmbbox-mediapress/media.php';
...
if ( isset($_GET['detached']) ) {
	...
	if ( !empty($lost) ) {
		...
		$orphans = $wpdb->get_results( "SELECT * FROM $wpdb->posts WHERE post_type = 'media' AND ID IN ($lost) LIMIT $start, 50" );
	} else {
		$start = ( $_GET['paged'] - 1 ) * 25;
		$orphans = $wpdb->get_results( "SELECT SQL_CALC_FOUND_ROWS * FROM $wpdb->posts WHERE post_type = 'media' AND post_parent < 1 LIMIT $start, 25" );
	...
	list($post_mime_types, $avail_post_mime_types) = $LMBBox_MediaPress->edit_media_query();
}
...
<?php
$type_links = array();
$_num_posts = (array) $LMBBox_MediaPress->count_media();
...
if ( ! is_singular() && ! isset($_GET['detached']) ) {
	$arc_query = "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $wpdb->posts WHERE post_type = 'media' ORDER BY post_date DESC";
